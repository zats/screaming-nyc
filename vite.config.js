  ...
  plugins: [
    {
      ...
      configureServer(server) {
        server.middlewares.use("/api/scan", async (req, res) => {
          try {
            ...
            const [sports, ticketedEvents] = await Promise.all([
              ...
              scanTicketedEvents(lat, lon, radius)
            ]);
        ...
        server.middlewares.use("/api/sports", async (req, res) => {
          try {
            const { lat, lon, radius } = scanParams(req);
        ...
        server.middlewares.use("/api/ticketed-source", async (req, res) => {
          try {
            const { lat, lon, radius, source } = scanParams(req);
            sendJson(res, 200, { events: await scanTicketedSource(source, lat, lon, radius) });
          } catch (error) {
            sendJson(res, 500, { error: error.message });
          }
        });
...
async function scanSports(lat, lon, radius) {
...
async function scanTicketedEvents(lat, lon, radius) {
  const results = await Promise.allSettled([
...
async function scanTicketedSource(source, lat, lon, radius) {
...
async function scanEventbrite(lat, lon, radius) {
  ...
  return events.flatMap((event) => {
    ...
    const time = eventDateTime(event.start_date, event.start_time, event.timezone);
...
function requestHeaders() {
  return {
    ...
    "user-agent": "Mozilla/5.0 why-are-people-screaming/0.1"
  };
}
